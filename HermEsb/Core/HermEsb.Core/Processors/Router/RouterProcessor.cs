...
namespace HermEsb.Core.Processors.Router
{
    ...
    public sealed class RouterProcessor : IProcessor, ISubscriber, ILoggable, IMonitorableRouter, IRouterErrorHandling
    {
        ...
        private readonly IInputGateway<MessageBus> _inputGateway;
        ...
        internal RouterProcessor(Identification identification, IInputGateway<MessageBus> inputGateway,
        ...
        public void Subscribe(SubscriptionKey messageType, Identification service, IOutputGateway<string> outputGateway)
        {
        ...
        private void MessageReceived(object sender, OutputGatewayEventHandlerArgs<MessageBus> args)
        {
            ...
            try
            {
                ...
                {
                    Logger.Warn(string.Format("Se ha publicado el Mensaje:{0}", args.SerializedMessage));
                ...
                if (!string.IsNullOrEmpty(args.Header.BodyType))
                {
                    ...
                    {
                        _routerOutputHelper.Reply(args.Header.CallStack.Peek().Identification, args.Header.Priority,
            ...
            catch (Exception ex)
            {
                ...
                OnOnRouterError(args.Message, ex);
            }

            InvokeOnMessageSent(args.Message != null
                                    ? new MonitorEventArgs {MessageCreatedAt = args.Message.Header.CreatedAt}
